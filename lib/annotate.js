  ...
  (function(jQuery) {
    ...
    Stanbol.getTextAnnotations = function(enhList) {
      ...
      res = _(enhList).filter(function(e) {
        return e.hasType("<" + ns.enhancer + "TextAnnotation>");
      });
    ...
    Stanbol.TextEnhancement.prototype = {
      ...
      getEntityEnhancements: function() {
        var entityAnnotations, rawList;
        entityAnnotations = Stanbol.getEntityAnnotations(this._enhList);
        rawList = _(entityAnnotations).filter(__bind(function(ann) {
          return ann.hasPropertyValue("dc:relation", this._enhancement.getSubject());
        }, this));
        ...
        this.entityCache = window.entityCache = {
          ...
          get: function(uri, scope, cb) {
            ...
            if (this._entities[uri] && this._entities[uri].status === "done") {
              ...
              cache = this;
              widget.options.vie.service('stanbol').connector.queryEntityHub(uri, function(entity) {
                if (!entity.status) {
                  if (entity.id !== uri) {
      ...
      _createSearchbox: function() {
        ...
        $('.search', this.searchEntryField).autocomplete({
          source: function(req, resp) {
            ...
            return widget.options.vie.service('stanbol').connector.findEntity("" + req.term + (req.term.length > 3 ? '*' : ''), function(entityList) {
              var entity, i, res;
